...
### 3.16 Dry-Run Execution contracts (Sprint 018 - Phase 8A)

Sprint 018 adds provider-independent contracts for a future BTCUSDT live-data dry-run. No runtime data
flow is implemented yet.

```text
Future Binance BTCUSDT feed
  -> ExecutionMode.DRY_RUN
  -> OrderIntent
  -> SimulatedOrder / SimulatedFill
  -> PaperPosition / PaperAccountSnapshot
  -> RuntimeStatusSnapshot / Heartbeat
  -> future read-only dashboard
```

**Boundary:** live market data is planned for Sprint 019; all orders, fills, positions and PnL are
simulated by contract. Sprint 018 does not add a provider adapter, runtime loop, persistence adapter,
AWS deployment or OVH dashboard.

**Entry points:** contract imports under `trading_framework.execution`; no application workflow yet.

**ADR:** ADR-0021

---

### 3.17 Binance BTC Futures Live Data Adapter (Sprint 019 - Phase 8A)

Sprint 019 adds a public Binance USD-M futures live-data adapter for the BTCUSDT demo. Binance-specific
schemas stay in `infrastructure/providers/binance/`; downstream runtime code consumes
provider-independent facts only.

```text
Binance USD-M public WebSocket
  -> routed stream specs (`/market` kline, `/public` bookTicker)
  -> aiohttp transport adapter
  -> reconnecting WebSocket client
  -> provider DTO parsing
  -> UTC normalization
  -> MarketBar / BestBidAskSnapshot / MarketFeedStatusSnapshot
  -> bounded smoke CLI output
```

**Streams:** `btcusdt@kline_1m` closed bars and `btcusdt@bookTicker` best bid/ask snapshots.

**Boundary:** no private Binance API, no account credentials, no order submission. Network-dependent
tests are opt-in through `TRADING_FRAMEWORK_RUN_BINANCE_NETWORK_SMOKE=1` and are skipped by default.
...
**Entry points:** `trading_framework.infrastructure.providers.binance.BinanceFuturesWebSocketClient`,
`AiohttpBinanceWebSocketConnector`, `run_binance_futures_feed_smoke`.

Integration test: `tests/integration/live_data/test_binance_futures_network_smoke.py` (opt-in).
...
| Execution read model | `execution.repositories`, `infrastructure.storage`, `scripts/execution` | `ExecutionStateRepository`, `JsonExecutionStateRepository`, `DynamoDbExecutionStateRepository`, `scripts/execution/show_execution_status.py` |
