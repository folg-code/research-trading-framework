...
The public `query_historical` list-returning path remains, so the debt is still active. A separate `query_historical_columnar` path now supports bounded columnar consumers; see the [Market Data workflow](../../reference/workflows/MARKET_DATA.md). The latter does not erase the object-materialization cost of the former.
