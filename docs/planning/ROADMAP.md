...
Research Capability Track
  ...
  Phase 10C — Neural Predictive Models            COMPLETE on sprint branch (Sprint 043; S044 next)
...
**Recommended next increment:** **Sprint 044 — predictive dashboard + IDEA-014 gate** (Phase 10C). Sprint 043 neural models is complete on `sprint/predictive-neural-models` (21/21; not yet merged to `main`). Sprint 042 tree-based models is complete on `main` (#335, 22/22). Phase 10A and Phase 10B are complete. Catalog follow-ons (wick / distance) remain in parallel. Deferred by default: Phase 4B/6B, Phase 8 Replay, PBO/CSCV ADR. Stage 3 (`available_at` column / lineage sidecar) and Stage 4 (`MarketFrame`) remain independently sequenced.
...
Phase 10C Sprint 043 COMPLETE on `sprint/predictive-neural-models` (21/21; not yet
merged to `main`); Sprint 044 next. Numbered `13A` to avoid
...
synthetic comparison) is complete on `sprint/predictive-neural-models`.
