...
Research Capability Track
  ...
  Phase 10A — Predictive Research Foundation      S039–S040 COMPLETE; S041 pending #325
...
**Recommended next increment:** **Sprint 042 — tree-based predictive models** (Phase 10B, Wave 0). Sprint 040 baselines are on `main` (#319). Sprint 041 report v1 waits on integration PR #325. Phase 10A is complete after #325. Catalog follow-ons (wick / distance) remain in parallel. Deferred by default: Phase 4B/6B, Phase 8 Replay, PBO/CSCV ADR. Stage 3 (`available_at` column / lineage sidecar) and Stage 4 (`MarketFrame`) remain independently sequenced.
...
**Status:** Phase 10A S039–S040 on `main`; S041 integration #325 pending; Phase 10B Wave 0 started. Numbered `13A` to avoid renumbering sections cited elsewhere.

Sprint 039 (dataset foundation: labelled matrix, purged walk-forward folds, persisted envelope,
leakage suite, ADR-0023) and Sprint 040 (estimator seam, sklearn baselines, run envelope, metrics,
CLIs) are implemented. Sprint 041 (offline HTML report) is implemented on
`sprint/predictive-research-report` (PR #325). Phase 10A is complete after #325. Sprint 042
