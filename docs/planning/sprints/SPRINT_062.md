...
| T004 | Package worker and status services and extend VPS Compose with private networking, read/write state volume only for the worker, read-only state access for status, health checks, restart policy and bounded logs/resources | T002–T003 | deploy + infrastructure | high | In review — PR open against `sprint/btc-futures-vps-dry-run` | [PR TBD](#) |
