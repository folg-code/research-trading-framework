...
| T002 | Implement and test the VPS runtime config/entry point, durable JSON repository wiring, continuous service lifetime and graceful recovery semantics while reusing existing execution logic unchanged | T001 (ADR-0035 ACCEPTED) | `application/execution`, `scripts/execution`, storage adapter | high | tester + reviewer APPROVE; CI green; ready to merge pending maintainer go-ahead | [#504](https://github.com/folg-code/research-trading-framework/pull/504) |
| T003 | Implement a bounded GET-only status service over the `ExecutionStateReader` port, with a sanitized versioned response, freshness semantics, health endpoint and no file-path/infrastructure leakage | T001 (ADR-0035 ACCEPTED); parallel with T002 after schema freeze | status application/service + tests | high | tester + reviewer APPROVE (after a reviewer-caught event-payload leak was fixed and re-verified); ready to merge pending maintainer go-ahead | [#505](https://github.com/folg-code/research-trading-framework/pull/505) |
