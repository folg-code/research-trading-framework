...
|---|---|---|---|---|---|---|
| T001 | `scaffold_component.py` generates a correct file skeleton + test stub for a new component ID, matching the existing hand-written pattern exactly | Approved sprint | `scripts/market_analysis` | standard | Done — `scripts/market_analysis/scaffold_component.py` + `tests/unit/scripts/test_scaffold_component_cli.py` (8 tests), ruff/ruff-format/mypy clean on both the tool and its generated output | PR TBD |
