...
| T004 | `check_promotion_readiness.py` runs all six D-P19-01 checks per named component ID and reports PASS/FAIL/NEEDS-REVIEW without mutating any file | Independent of T001-T003 | `scripts/market_analysis` | standard | Done — 22 tests (fake-component unit tests per check + real-registry integration tests); self-review fixed a malformed-id crash and a redundant per-id registry rebuild before opening the PR; verified against the real repo | [#554](https://github.com/folg-code/research-trading-framework/pull/554) |
| T005 | `structure.opening_gap` is scaffolded via T001-T003, its real formula is implemented (gap = current bar open minus prior bar close, plus a normalized/percentage variant if the catalog convention calls for one — implementer confirms against `structure.level_distance`'s ATR-normalization precedent), its contract test passes, its catalog entry is completed (formula, warm-up, zero-denominator/first-bar convention), and `check_promotion_readiness.py` reports it PASS | T001-T004 | `market_analysis/components/structure` | standard | Done — `gap_atr = (open - prior_close) / atr`; 3 behavioral tests + scaffolded identity/registration tests; catalog entry complete; `check_promotion_readiness.py` reports all applicable checks PASS; self-review fixed one reuse finding (wrong dependency-lookup helper) before opening the PR | [#555](https://github.com/folg-code/research-trading-framework/pull/555) |
