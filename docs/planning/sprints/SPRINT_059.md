...
| T003 | Extend app-boundary enforcement to `apps/dashboard/pages/*.py`; prove the portfolio code imports no framework engines, execution, providers or ML libraries; update PRB-022 disposition | T001 | `tests/unit/test_apps_boundaries.py` | standard | In review | pending |
| T004 | Implement the content loader, required metadata validation, restricted rendering and stable-slug routing contract, with missing/invalid-content behavior covered by tests | T001, T002 | `apps/dashboard` content/navigation | standard | Ready | — |
