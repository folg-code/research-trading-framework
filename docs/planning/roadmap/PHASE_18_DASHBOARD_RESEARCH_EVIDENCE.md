...
| 18D Robustness Research evidence view | Rolling-window robustness (correct formula, not the historical erroneous one) | **APPROVED (maintainer, 2026-09-22)** — [PRD](../../product/PRD-dashboard-robustness-research-evidence.md); Wave 0 complete and accepted ([decisions](PHASE_18D_WAVE0_DECISIONS.md)); [Sprint 075](../sprints/SPRINT_075.md) (Milestone 1: fold geometry, overlap and stability) open. Grill-me investigation found no trace of the vision doc's "historical erroneous formula" anywhere in the repo (treated as a caution, not a bug to locate); found the real experiment already uses `window_mode=ROLLING` with a 24-day fold-overlap that is computed correctly but never disclosed. Milestone 2 (dashboard UI) not opened yet. |
