"""Integration tests for Strategy Research dashboard view model builder."""
...
def _write_published_dataset(storage_root: Path, *, csv_path: Path) -> DatasetRef:
    ...
    dataset_id = DatasetId(
        ...
        provider="csv",
        source_id="s014-strategy-dashboard",
    )
...
def test_build_strategy_dashboard_view_model_from_persisted_run(
    tmp_path: Path,
    ...
    assert view_model.run_id == result.run_id
    assert view_model.overview.trade_count == len(result.trades)
    assert view_model.metadata.bar_count == len(view_model.bars)
    assert view_model.metadata.bar_count > 0
    assert view_model.metadata.effective_from_utc is not None
    assert view_model.metadata.effective_to_utc is not None
    ...
    assert "performance" in payload["sections"]
    assert "conditional" in payload["sections"]

    if view_model.overview.trade_count > 0:
        assert len(view_model.trades) == view_model.overview.trade_count
        assert view_model.overview.profit_factor is not None or view_model.overview.trade_count == 0
