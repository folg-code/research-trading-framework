"""Tests for Market Model evaluation."""

import math
from collections.abc import Callable

from trading_framework.market_analysis import OutputId
from trading_framework.market_analysis.assembly.frame import AnalysisFrame
from trading_framework.market_analysis.components.volatility import VolatilityStateComponent
from trading_framework.market_model import MarketModelDefinition, MarketModelEvaluator
from trading_framework.model_expression import (
    CompareExpression,
    ComparisonOperator,
    ComponentOutputReference,
    MarketField,
    MarketFieldReference,
)
from trading_framework.time.models.timeframe import Timeframe


def test_market_model_evaluates_dense_result(
    build_test_frame: Callable[..., AnalysisFrame],
) -> None:
    component = VolatilityStateComponent()
    frame = build_test_frame(
        columns={
            "vol_state": (math.nan, 0.0, 1.0, 1.0, 0.0),
        }
    )
    definition = MarketModelDefinition(
        market_model_id="high_volatility",
        expression=CompareExpression(
            operand=ComponentOutputReference(
                component_id=component.component_id,
                parameters=component.parameter_schema.canonicalize(
                    {"period": 14, "threshold": 5.0}
                ),
                output_id=OutputId("state"),
                alias="vol_state",
            ),
            operator=ComparisonOperator.EQ,
            value=1.0,
        ),
    )

    result = MarketModelEvaluator().evaluate(
        definition,
        frame,
        evaluation_timeframe=Timeframe("1m"),
    )

    assert result.columns == [
        "timestamp",
        "available_at",
        "model_result",
        "market_model_id",
    ]
    assert result["market_model_id"][0] == "high_volatility"
    assert result["model_result"].is_null()[0]
    assert result["model_result"].to_list()[1:] == [False, True, True, False]


def test_market_model_supports_market_field_expression(
    build_test_frame: Callable[..., AnalysisFrame],
) -> None:
    frame = build_test_frame(columns={"close": (100.0, 101.0, 99.0)})
    definition = MarketModelDefinition(
        market_model_id="close_above_100",
        expression=CompareExpression(
            operand=MarketFieldReference(field=MarketField.CLOSE),
            operator=ComparisonOperator.GT,
            value=100.0,
        ),
    )

    result = MarketModelEvaluator().evaluate(
        definition,
        frame,
        evaluation_timeframe=Timeframe("1m"),
    )

    assert result["model_result"].to_list() == [False, True, False]
