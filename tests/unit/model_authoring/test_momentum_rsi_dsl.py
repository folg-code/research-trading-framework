"""Authored momentum.rsi models evaluate on the fixture harness path."""
...
def _write_published_dataset(storage_root: Path, *, csv_path: Path) -> DatasetRef:
    ...
    dataset_id = DatasetId(
        ...
        provider="csv",
        source_id="s051-momentum-rsi-dsl",
    )
...
def test_rsi_returns_operand_usable_in_a_condition() -> None:
    operand = momentum.rsi(period=14, timeframe=None)
    assert isinstance(operand, Operand)
    condition = operand > 70.0
    assert condition is not None


def test_authored_rsi_evaluates_on_fixture(
    tmp_path: Path,
    ...
    rsi_extreme = market_model(
        "rsi_extreme",
        when=(momentum.rsi(period=14) > -1.0),
    )
    ...
    result = evaluate_models(
        EvaluateModelsRequest(
            ...
            market_models=(rsi_extreme.definition,),
        )
    )

    assert result.analysis.frame is not None
    assert "rsi_extreme" in result.market_model_results
    computed_ids = {
    ...
    assert "momentum.rsi" in computed_ids
    frame = result.market_model_results["rsi_extreme"]
    assert frame.height > 0
    assert frame["market_model_id"][0] == "rsi_extreme"
