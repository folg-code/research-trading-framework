"""Authored statistics.return_autocorrelation models evaluate on the fixture harness path."""
...
def _write_published_dataset(storage_root: Path, *, csv_path: Path) -> DatasetRef:
    ...
    dataset_id = DatasetId(
        ...
        provider="csv",
        source_id="s051-return-autocorrelation-dsl",
    )
...
def test_return_autocorrelation_returns_operand_usable_in_a_condition() -> None:
    operand = statistics.return_autocorrelation(period=60, lag=1)
    assert isinstance(operand, Operand)
    condition = operand < 0.0
    assert condition is not None


def test_authored_return_autocorrelation_evaluates_on_fixture(
    tmp_path: Path,
    ...
    mean_reverting = market_model(
        "mean_reverting",
        when=(statistics.return_autocorrelation(period=20, lag=1) >= -1.0),
    )
    ...
    result = evaluate_models(
        EvaluateModelsRequest(
            ...
            market_models=(mean_reverting.definition,),
        )
    )

    assert result.analysis.frame is not None
    assert "mean_reverting" in result.market_model_results
    computed_ids = {
    ...
    assert "statistics.return_autocorrelation" in computed_ids
    frame = result.market_model_results["mean_reverting"]
    assert frame.height > 0
    assert frame["market_model_id"][0] == "mean_reverting"
