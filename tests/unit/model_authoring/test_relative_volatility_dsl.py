"""Authored volatility.relative_volatility* models evaluate on the fixture harness path."""
...
from trading_framework.model_authoring import market_model, volatility
...
def _write_published_dataset(storage_root: Path, *, csv_path: Path) -> DatasetRef:
    ...
    dataset_id = DatasetId(
        ...
        provider="csv",
        source_id="s051-relative-volatility-dsl",
    )
...
def test_relative_volatility_returns_operand_usable_in_a_condition() -> None:
    operand = volatility.relative_volatility(period=20, baseline_period=100)
    assert isinstance(operand, Operand)
    condition = operand > 0.0
    assert condition is not None


def test_relative_volatility_ratio_returns_operand() -> None:
    assert isinstance(volatility.relative_volatility_ratio(), Operand)


def test_authored_relative_volatility_evaluates_on_fixture(
    tmp_path: Path,
    ...
    elevated_relative_vol = market_model(
        "elevated_relative_vol",
        when=(volatility.relative_volatility_ratio(period=10, baseline_period=30) >= 0.0),
    )
    ...
    result = evaluate_models(
        EvaluateModelsRequest(
            ...
            market_models=(elevated_relative_vol.definition,),
        )
    )

    assert result.analysis.frame is not None
    assert "elevated_relative_vol" in result.market_model_results
    computed_ids = {
    ...
    assert "volatility.relative_volatility" in computed_ids
    frame = result.market_model_results["elevated_relative_vol"]
    assert frame.height > 0
    assert frame["market_model_id"][0] == "elevated_relative_vol"
