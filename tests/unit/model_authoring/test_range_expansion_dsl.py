"""Authored volatility.range_expansion models evaluate on the fixture harness path."""
...
def _write_published_dataset(storage_root: Path, *, csv_path: Path) -> DatasetRef:
    ...
    dataset_id = DatasetId(
        ...
        provider="csv",
        source_id="s048-range-expansion-dsl",
    )
...
def test_range_expansion_returns_operand_usable_in_a_condition() -> None:
    operand = volatility.range_expansion(period=14, timeframe=None)
    assert isinstance(operand, Operand)
    condition = operand > 1.5
    assert condition is not None


def test_authored_range_expansion_evaluates_on_fixture(
    tmp_path: Path,
    ...
    wide_range_bar = market_model(
        "wide_range_bar",
        when=(volatility.range_expansion(period=14) > -100.0),
    )
    ...
    result = evaluate_models(
        EvaluateModelsRequest(
            ...
            market_models=(wide_range_bar.definition,),
        )
    )

    assert result.analysis.frame is not None
    assert "wide_range_bar" in result.market_model_results
    computed_ids = {
    ...
    assert "volatility.range_expansion" in computed_ids
    assert "volatility.true_range" in computed_ids
    assert "volatility.atr" in computed_ids
    frame = result.market_model_results["wide_range_bar"]
    assert frame.height > 0
    assert frame["market_model_id"][0] == "wide_range_bar"
