"""Authored trend.slope models evaluate on the fixture harness path."""
...
def test_authored_slope_evaluates_on_fixture(
    tmp_path: Path,
    ...
    result = evaluate_models(
        EvaluateModelsRequest(
            ...
            market_models=(positive_slope.definition,),
        )
    )

    assert result.analysis.frame is not None
    assert "positive_slope" in result.market_model_results
    computed_ids = {
    ...
    assert "trend.slope" in computed_ids
    frame = result.market_model_results["positive_slope"]
    assert frame.height > 0
    assert frame["market_model_id"][0] == "positive_slope"
