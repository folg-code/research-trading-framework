...
def render_predictive_research_report(
    ...
    ``selection.json``, ``leaderboard.json``, ``learning_curves.json``, and
    ``window_accounting.json``. Never fits, predicts, or rebuilds. Never
    ...
    view = build_predictive_report_view_model(
        PredictiveReportSource(
            ...
            learning_curves=_optional_sidecar(
                predictive_research_run_learning_curves_path(
                    request.storage_root, request.run_ref.run_id
                ),
                LearningCurves.from_dict,
            ),
            window_accounting=_optional_sidecar(
                predictive_research_run_window_accounting_path(
                    request.storage_root, request.run_ref.run_id
                ),
                WindowAccounting.from_dict,
            ),
        ),
