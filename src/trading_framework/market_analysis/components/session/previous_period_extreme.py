"""Session Previous Period Extreme Market Analysis component."""
...
_COMPONENT_ID = ComponentId("session.previous_period_extreme")
_COMPONENT_VERSION = ComponentVersion("1.0.0")
_IMPLEMENTATION_ID = ImplementationId("numpy.previous_period_extreme")
...
class PreviousPeriodExtremeComponent:
    """Causal previous, fully-closed day or week period's high/low.

    ``value`` is the last *fully closed* ``period`` ("day" or "week")'s
    maximum (``side="high"``) or minimum (``side="low"``) of the bar's own
    ``side`` column, held constant through the whole following period until
    the next period closes. ``NaN`` before any period has closed (the first
    period has no previous period). Never includes any bar from the
    still-open current period -- the defining causal-only requirement for
    every ``session.*`` component.
    """
...
class NumpyPreviousPeriodExtremeImplementation:
    """NumPy Previous Period Extreme backend."""
    ...
    def compute(
        ...
        outputs: dict[OutputId, OutputSeries] = {
            _VALUE: ndarray_to_output_series(arrays.previous),
        }
...
__all__ = ["NumpyPreviousPeriodExtremeImplementation", "PreviousPeriodExtremeComponent"]
