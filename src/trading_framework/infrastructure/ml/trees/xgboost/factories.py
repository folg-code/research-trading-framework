"""Lazy XGBoost family factories. Import xgboost inside each factory only."""

from __future__ import annotations

from trading_framework.infrastructure.ml.trees.xgboost.adapter import XGBoostPredictiveEstimator
from trading_framework.research.predictive.errors import PredictiveExtraError
from trading_framework.research.predictive.estimators import EstimatorSpec, PredictiveEstimator
from trading_framework.research.predictive.preprocessing import PreprocessingSpec

_TREES_EXTRA = "ml-trees"


def _import_xgboost() -> object:
    import xgboost

    return xgboost


def _require_xgboost(family_id: str) -> None:
    try:
        _import_xgboost()
    except ImportError as exc:
        msg = (
            f"estimator family {family_id!r} requires optional extra {_TREES_EXTRA!r}; "
            f"install with `uv sync --extra {_TREES_EXTRA}`"
        )
        raise PredictiveExtraError(msg) from exc


def create_xgboost_regressor(
    spec: EstimatorSpec,
    *,
    preprocessing: PreprocessingSpec | None = None,
) -> PredictiveEstimator:
    _require_xgboost("xgboost.regressor")
    return XGBoostPredictiveEstimator(spec, preprocessing=preprocessing)


def create_xgboost_classifier(
    spec: EstimatorSpec,
    *,
    preprocessing: PreprocessingSpec | None = None,
) -> PredictiveEstimator:
    _require_xgboost("xgboost.classifier")
    return XGBoostPredictiveEstimator(spec, preprocessing=preprocessing)
